...
3. Dispatch the OTHER-vendor sub-agent as reviewer (Claude built it → `codex`;
   Codex built it → `claude_code`). Use a task-based title such as
...
7. **HARD CAP — at most 3 review rounds per PR.** A round = one reviewer pass
   plus the implementer's fix attempt (steps 1–5). Track the round count for
   the PR in the registry. If the PR still has blocking issues after the 3rd
   round, STOP — do not dispatch a 4th review — and escalate to the human with
   the PR URL and the remaining blocking findings so they can decide (take
   over, accept-with-caveats, or abandon). Never loop unbounded.

## Notes
- Cross-review requires a reviewer from a DIFFERENT vendor than the implementer,
  so it needs BOTH workers available (per the roster preflight). If only one of
  the two — `claude_code` or `codex` — is available on the machine, you CANNOT
  run independent cross-vendor review: don't dispatch a reviewer that can't
  boot, say so explicitly, and pull in the human at the plan gate.
...
- Review is a coding sub-agent (`claude_code`/`codex`) dispatched with
  `purpose: "review"` — the OTHER vendor from the one that built the diff. It
