...
2. Dispatch each task to `claude_code` or `codex`:
   ...
   when the task is primarily external/document search. For a second
   independent lens, dispatch the same task to BOTH vendors. Any worker takes an
   optional `args.model` (`sys_list_models` shows what each worker can run; an invalid
